use crate::interop::javascript::log::console_log;
use crate::lexer::types::brackets::{is_closing_bracket_round, is_opening_bracket_round};
...
impl GrammarLike for FunctionDeclarationGrammar {
    ...
    fn find_match_end(&self, nodes: &[TreeNode], start_index: usize) -> Option<usize> {
        console_log(&format!("Start: {}",start_index));
        let args_start = find_args_start(start_index, &nodes)?;
        console_log(&format!("Args start: {}",args_start));
